...
client.on('message', async msg => { 
	...
	if (command === `play`) {
        ...
        if (!voiceChannel) return msg.channel.send("Please Join Voice Room");
        
        const permissions = voiceChannel.permissionsFor(msg.client.user);
        
        if (!permissions.has('CONNECT')) {

			return msg.channel.send("i Don't Have Permission To Join");
        }
        
		if (!permissions.has('SPEAK')) {

			return msg.channel.send("i Can't Start Music");
		}

		if (!permissions.has('EMBED_LINKS')) {

			return msg.channel.sendMessage("i Don't Have Premission Linkss")
		}
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			return msg.channel.send(`> Start Now :${playlist.title} :musical_note: `);
		} else {
...
/////////////////					
					try {
						...
						console.error(err);
						return msg.channel.send('File Search Name Music');
                    }
...
client.login("NjU3MTU1NTUwNjMwMzc5NTIw.XftF4w.-PPO6nN5V_I6RxFmKDwsUfTaX9A");
